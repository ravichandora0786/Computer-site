...
const CourseDetail = () => {
  ...
  const handlePageMastery = async (isFinal = false) => {
    ...
    // Logic for periodic heartbeat (decrementing from requirements)
    const secondsToSync = isFinal ? 0 : 10; 
    ...
    try {
      const response = await httpRequest.post("/lesson-progress/update-page-progress", {
        ...
        lessonId,
        courseId: id,
        secondsIncrement: 10 // Heartbeat interval
      });
  ...
  const startPageTimer = (lessonId, page) => {
    ...
    if (!isAlreadyDone) {
      const required = (page.required_time || 1) * 60;
  ...
  return (
    <div className="bg-white selection:bg-primary/20">
      ...
      <section className="bg-gray-50/50 pb-40">
        <div className="container max-w-full mx-auto px-4 md:px-12">
          ...
          <div className="flex justify-center -mt-8 relative z-40 mb-16">
             <div className="bg-white p-1.5 rounded-full shadow-xl border border-gray-100 flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              <AnimatePresence mode="wait">
                {activeTab === "overview" && (
                  ...
                  >
                    <div>
                       ...
                       <h2 className="text-2xl font-black text-main uppercase italic mb-6 tracking-tight">Executive Summary</h2>
                       <div 
                         className="text-gray-500 font-medium text-sm leading-relaxed space-y-4 prose max-w-none"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                       {[
                         ...
                         <div key={i} className="bg-white p-6 rounded-2xl border border-gray-100 flex items-start gap-5">
                            <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-primary border border-gray-100">
                               <MdVerified size={20} />
                            </div>
                            <div>
                               <h4 className="text-base font-black text-main uppercase italic mb-1 tracking-tight">{feat.title}</h4>
                               <p className="text-gray-500 font-medium italic text-xs">{feat.desc}</p>
                            </div>
                         </div>
                       ))}
                    </div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-8 px-2">
                       <div>
                          ...
                          <h2 className="text-2xl font-black text-main uppercase italic tracking-tight">Curriculum</h2>
                       </div>
                       <div className="text-right">
                          <p className="text-[9px] font-black uppercase text-gray-400 mb-0.5 italic">Total</p>
                          <p className="text-lg font-black text-main italic">{course.modules?.reduce((acc, m) => acc + (m.lessons?.length || 0), 0)} Sessions</p>
                       </div>
                    </div>
                                      ...
                                      <div key={lesson.id} className="bg-white rounded-xl border border-gray-50 overflow-hidden shadow-sm">
                                        ...
                                        >
                                          <div className="flex items-center gap-4">
                                            <div className="w-8 h-8 bg-white border border-gray-100 rounded-lg flex items-center justify-center text-primary italic font-black text-[9px]">
                                              0{lIdx + 1}
                                            </div>
                                            <h5 className="text-sm font-black uppercase italic tracking-tight text-main">{lesson.title}</h5>
                                          </div>
                                          <div className="flex items-center gap-4">
                                                ...
                                                <div className="flex flex-col xl:flex-row min-h-[300px]">
                                                    ...
                                                    {lesson.pages.map((page) => {
                                                      ...
                                                      return (
                                                        <button
                                                          key={page.id}
                                                          disabled={!isPageDone && activePageId !== page.id && isTimerRunning}
                                                          ...
                                                          className={clsx(
                                                            ...
                                                            !isPageDone && activePageId !== page.id && isTimerRunning && "opacity-50 cursor-not-allowed"
                                                          )}
                                                        >
                ...
                {activeTab === "instructor" && (
                  <motion.div 
                    ...
                    className="p-10 rounded-3xl bg-white border border-gray-100 shadow-xl flex flex-col xl:flex-row gap-8 items-center"
                  >
                     ...
                     <div className="flex-1 text-center xl:text-left">
                        ...
                        <h2 className="text-3xl font-black text-main uppercase italic mb-4 tracking-tight">{course.author_details?.user_name || "Nexus Mentor"}</h2>
                        <p className="text-gray-500 text-lg font-medium leading-relaxed italic mb-8 max-w-2xl">Industrial craftsmanship specialized in digital architecture.</p>
                        <div className="flex justify-center xl:justify-start gap-10">
                           <div>
                              <div className="text-3xl font-black text-main italic">12K+</div>
                              <div className="text-[8px] font-black text-gray-400 uppercase mt-1">Alumni</div>
                           </div>
                           <div>
                              <div className="text-3xl font-black text-main italic">4.9</div>
            ...
            <div className="lg:col-span-4">
              <div className="lg:sticky lg:top-24 space-y-6">
                <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-gray-100 shadow-xl p-5 overflow-hidden group">
                   <div className="aspect-video rounded-xl overflow-hidden mb-5 relative group cursor-pointer shadow-md">
                      <img src={thumbnailUrl} className="w-full h-full object-cover transition-transform duration-[1.5s] group-hover:scale-105" alt="Preview" />
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
                         <MdPlayCircle size={48} className="text-white drop-shadow-xl" />
                      </div>
                   </div>

                   <div className="flex items-baseline gap-2 mb-6 border-b border-gray-50 pb-4">
                      <span className="text-3xl font-black text-main italic tracking-tight">
                   ...
                   <div className="space-y-3">
                      <h5 className="text-[8px] font-black text-main uppercase italic border-b border-gray-50 pb-2">Assets:</h5>
                      {[
